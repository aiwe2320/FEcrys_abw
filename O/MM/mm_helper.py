...
def update_RBTorsionForce_(_force, _lam, deepcopy=False):
    ...
    for i in range(force.getNumTorsions()):
        ...
        #assert c0(0,...,5).unit == unit.kilojoule/unit.mole
        new_c0 = c0*_lam
        new_c1 = c1*_lam
        new_c2 = c2*_lam
        new_c3 = c3*_lam
...
def update_NonbondedForce_(_force, _lam, deepcopy=True):
    ...
    for i in range(force.getNumExceptions()):
        ...
        pi,pj,qiqj,sigmaij,eps = old_params
        assert eps.unit == unit.kilojoule/unit.mole ; new_eps = eps*_lam
        assert qiqj.unit == unit.elementary_charge**2 ; new_qiqj = qiqj*_lam
        new_params = [i,pi,pj,new_qiqj,sigmaij,new_eps]
...
def update_CustomNonbondedForce_(_force, _lam, deepcopy=True):
    ...
    ## this is reached in tip4pICE because it has this custom force.

    ## not sure what these are, but param1*(_lam**0.5) seems to make u(lam) linear in lam.

    ## At lambdas 0 and 1 these details do not make a difference.
    ## Using BAR instead of TI is also better for this reason.
    ## if u(0) and u(1) are correct, which is easy to check,
    ## the rest is taken care of by BAR and the SE estimates it gives.
    ## [Also, potential energy overlaps, when plotted between all pairs of lambdas,
    ## correlate well with SE from BAR.]
    
    for i in range(force.getNumParticles()):
        old_params = force.getParticleParameters(i)
        param1, param2 = old_params
        new_param1 = param1*(_lam**0.5) # ! check if (eps or sigma), this may depend on (FF, combination rules)
        new_params = [new_param1, param2]
...
def put_lambda_into_system_(system,
    ...
    # deepcopy does not work (crashing) and system cant be simulation.system
    # this means that have to initialised from PDB from scratch,
    # every time when changing lambda (for sim or eval).
    # each lambda is a modified version of a seperate instance of system, attached to its own 
    # seperate instance of simulation.
