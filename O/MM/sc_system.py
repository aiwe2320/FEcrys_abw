...
"""
def TEST_OPLS_LJ_succinic_acid_(sc):
    itp_file_path = str(sc.itp_file_mol.absolute())
    pdb_file_path = str(sc._single_mol_LigParGen_pdb_file_.absolute())
    n_mol = sc.n_mol

    file_in = open(itp_file_path, 'r')
    S = [] ; E = [] ; Q = []
    for line in file_in:
        string = [x for x in line.split(' ') if x != '']
        if 'opls' in string[0]:
            S.append(float(string[5]))
            E.append(float(string[6]))
        try:
            if 'opls' in string[1]:
                Q.append(float(string[6]))
        except: pass
    file_in.close()
    Q = np.array(Q) ; Q -= Q.mean()
    '''
    Q = np.array([0.4428071428571429,-0.5239928571428571,0.5023071428571428,-0.16339285714285715,-0.5235928571428571,
                  -0.16339285714285715,0.13270714285714286,0.13270714285714286,0.5025071428571428,
                    0.13270714285714286,0.13270714285714286,-0.5235928571428571,-0.5231928571428571,0.44270714285714285,])
    '''
    Sigma = np.array([0, 0.312, 0.355, 0.35, 0.296, 0.35, 0.25, 0.25, 0.355, 0.25, 0.25, 0.312, 0.296, 0.0])
    Eps   = np.array([0, 0.711279, 0.29288, 0.276144, 0.87864, 0.276144, 0.12552, 0.12552, 0.29288, 0.12552, 0.12552, 0.71128, 0.87864, 0.0])


    #Sigma = np.array(S) ; Eps = np.array(E) ; Q = np.array(Q) ; Q -= Q.mean()
    Q = np.concatenate([Q]*n_mol,axis=0)
    Sigma = np.concatenate([Sigma]*n_mol,axis=0)
    Eps = np.concatenate([Eps]*n_mol,axis=0)

    system = sc.system
    forces = {system.getForce(index).__class__.__name__: system.getForce(index) for index in range(system.getNumForces())}
    nonbonded_force = forces['NonbondedForce']
    custom_nonbonded_force = forces['CustomNonbondedForce']
    # custom_nonbonded_force.getUseLongRangeCorrection() -> True

    INC_NB = np.eye(scO.N)*0.0

    _q_i = []
    for i in range(nonbonded_force.getNumParticles()):
        q_i, sigma_i, eps_i = [x._value for x in nonbonded_force.getParticleParameters(i)]
        #print(i, 'zero:', q_i-Q[i]) -> Q_ij_mask = np.outer(Q,Q)
        _q_i.append(q_i)
        
        if eps_i > 0.0:
            print(i, q_i-Q[i]) # nothing printed; no LJ here; good.
            INC_NB[i,:] += 1
            INC_NB[:,i] += 1
        else: pass
            
    _q_i = np.array(_q_i)
    Q_ij_mask = np.outer(_q_i,_q_i) - np.diag(_q_i**2)
    
    print('testing 1-4 interactions:')
    for _ in range(nonbonded_force.getNumExceptions()):
        i, j, qij, sigij, epsij = nonbonded_force.getExceptionParameters(_)
        qij = qij._value
        sigij = sigij._value
        epsij = epsij._value
        ...
        if epsij>0.0:
            print('3 zeros:', qij-0.5*Q[i]*Q[j],  sigij - np.sqrt(Sigma[i]*Sigma[j]),  epsij - 0.5*np.sqrt(Eps[i]*Eps[j]) )
            INC_NB[i,j] += 0.5
            INC_NB[j,i] += 0.5
    
    plt.matshow(INC_NB)
    plt.title('f_ij=0.5 factor only (i.e., 1-4)')
    plt.colorbar(fraction=0.025)
    plt.show()
    
    ####
    
    print('custom_nonbonded_force function:', custom_nonbonded_force.getEnergyFunction())
    '''
    all of the f_ij = 1.0, and nothing else should be here. 
    
    4*eps_ij = 4*sqrt(eps_i*eps_j) = sqrt(4*eps_i)*sqrt(4*eps_j)
    sig_ij = sqrt(sig_i*sig_j) = sqrt(sig_i) * sqrt(sig_j)
    
    custom_nonbonded_force function:
    epsilon1*epsilon2*(sigr6^2-sigr6); sigr6=sigr2*sigr2*sigr2; sigr2=(sigc/r)^2; sigc=sigma1*sigma2
    
    \therefore the test must show a match in the following way:
    epsilon1 = sqrt(4*eps_1)
    sigma1   = sqrt(sig_1)   
    
    '''
    print('testing >(1-4) interactions:')
    
    INC_cNB = np.eye(scO.N)*0.0 + 1.0
    for i in range(custom_nonbonded_force.getNumParticles()):
        eps_i, sig_i = custom_nonbonded_force.getParticleParameters(i)
        print('2 zeros:', np.sqrt(Sigma[i]) - sig_i, np.sqrt(Eps[i])*np.sqrt(4) - eps_i)
    
    for k in range(custom_nonbonded_force.getNumExclusions()):
        i,j = custom_nonbonded_force.getExclusionParticles(k)
        INC_cNB[i,j] -= 1.0
        INC_cNB[j,i] -= 1.0
    
    plt.matshow(INC_cNB)
    plt.title('f_ij = 1.0 factor only (i.e., >(1-4) long range interactions)')
    plt.colorbar(fraction=0.025)
    plt.show()
    
    plt.matshow(INC_NB + INC_cNB)
    plt.title('the two masks add (all f_ij should be \in [0,1], with 0 only for 1-2, 1-3 interactions (i.e., atom pairs <3 bonds away have no LJ))')
    plt.colorbar(fraction=0.025)
    plt.show()
    '''
    # this is not the actual f_ij, because some epsilons were zero already
    # that it, had enougn of this.
    ...
    whithin_3bonds_away = np.eye(n_atoms_mol)*0.0
    exactly_3bonds_away = np.eye(n_atoms_mol)*0.0
    more_than_3bonds_away = np.eye(n_atoms_mol)*0.0
    for i in range(n_atoms_mol):
        for j in range(n_atoms_mol):
            n_bonds_in_between = len(nx.shortest_path(gr, i,j))
            if n_bonds_in_between <=3:
                whithin_3bonds_away[i,j] = 1
            else: pass
            if n_bonds_in_between == 3:
                exactly_3bonds_away[i,j] = 1
            else: pass
            if n_bonds_in_between > 3:
                more_than_3bonds_away[i,j] = 1
            else: pass

    fij_mol = whithin_3bonds_away*0.0 + exactly_3bonds_away*0.5 + more_than_3bonds_away*1.0

    fij = np.eye(len(AM)*n_mol)*0 + 1.0
    for i in range(n_mol):
        fij[n_atoms_mol*i:n_atoms_mol*(i+1),n_atoms_mol*i:n_atoms_mol*(i+1)] = fij_mol
    
    plt.matshow(np.abs(np.where(fij<0.4,1,0) - np.where((INC_NB + INC_cNB - np.eye(N))<0.4,1,0)))
    plt.colorbar(fraction=0.025)
    plt.show()
    '''
